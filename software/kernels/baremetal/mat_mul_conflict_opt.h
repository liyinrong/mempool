...
void mat_mul_unrolled_4x4_noc_opt_parallel_extra_reg_asm(int32_t const *__restrict__ A,
                                               int32_t const *__restrict__ B,
                                               int32_t *__restrict__ C,
                                               uint32_t M, uint32_t N,
                                               uint32_t P, uint32_t id,
  ...
  for (uint32_t i = (((id % 256) / c) * 16 + (id / 256) * 4); i < M;
    ...
    for (uint32_t _j = 0; _j < c_len; _j += 4) {
      ...
      __asm__ volatile(
          ...
          "p.lw x14, 4(%[addr_b]!) \n\t"
          "p.lw x15, %[P_3](%[addr_b]!) \n\t" // Increment by P-3
          // clear all the mac rd registers
          "li x16, 0 \n\t"
          "li x17, 0 \n\t"
          "li x18, 0 \n\t"
          "li x19, 0 \n\t"
          ...
          "li x30, 0 \n\t"
          "li x31, 0 \n\t"
          ".word 0x2274700B \n\t" // p.lw.rrpost.s  sx0, x7(x8!)   ; sx0 <- x3
          ".word 0x2274708B \n\t" // p.lw.rrpost.s  sx1, x7(x8!)   ; sx1 <- x4
          ".word 0x2274710B \n\t" // p.lw.rrpost.s  sx2, x7(x8!)   ; sx2 <- x10
          ".word 0x2294718B \n\t" // p.lw.rrpost.s  sx3, x9(x8!)   ; sx3 <- x11 // Increment by -3N+1
          ".word 0x0043320B \n\t" // p.lw.irpost.s  sx4, 4(x6!)    ; sx4 <- x12
          ".word 0x0043328B \n\t" // p.lw.irpost.s  sx5, 4(x6!)    ; sx5 <- x13
          ".word 0x0043330B \n\t" // p.lw.irpost.s  sx6, 4(x6!)    ; sx6 <- x14
          ".word 0x3F43338B \n\t" // p.lw.irpost.s  sx7, 1012(x6!); sx7 <- x15 // Increment by P-3
          // Inner loop
          "loop: \n\t"
          "p.mac x16,  x3, x12 \n\t"
          "p.mac x17,  x3, x13 \n\t"
          ...
          ".word 0x42712DB3 \n\t" // p.mac.s x27, sx2, sx7
          ".word 0x4271AFB3 \n\t" // p.mac.s x31, sx3, sx7
          ".word 0x3F43338B \n\t" // p.lw.irpost.s  sx7, 0x3F4(x6!); sx7 <- x15 // Increment by P-3
          ".word 0x2274710B \n\t" // p.lw.rrpost.s  sx2, x7(x8!)   ; sx2 <- x10
          ".word 0x2294718B \n\t" // p.lw.rrpost.s  sx3, x9(x8!)   ; sx3 <- x11 // Increment by -3N+1
          "bne %[addr_b], x1, loop \n\t"
          // Set up for second inner loop
